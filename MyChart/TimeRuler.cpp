...
void TimeRuler::wheelEvent(QWheelEvent *event)
{
    ...
    double mouseOnTimePoint = (mousePos - winContext.viewWindowB)
                              / winContext.viewWindowA;
    if (dic > 0) {
        winContext.viewWindowA *= 1.22;
    } else {
        winContext.viewWindowA /= 1.22;
    }
    if (winContext.viewWindowA > 500) {
        winContext.viewWindowA = 500;
    }
    if (winContext.viewWindowA < 1.0e-10) {
        winContext.viewWindowA = 1.0e-10;
    }
    winContext.viewWindowB = (mouseOnTimePoint * winContext.viewWindowA - mousePos) * -1.;
    update();
}
...
void TimeRuler::paintEvent(QPaintEvent *event)
{
    ...
    leftTime = (long long) ((-winContext.viewWindowB) / winContext.viewWindowA);
    rightTime = (long long) ((double(right) - winContext.viewWindowB)
                                       / winContext.viewWindowA);
    ...
    fm = p.fontMetrics();
    // 每个最小格站的像素点
    int um = (int) (double) (minStep * winContext.viewWindowA);
    // tick值的宽度
    ...
    // 自然数据模式
    natureShow = 1;
    if (winContext.mouseLocationY < 12) {
        natureShow = 0;
    }
    for (long long drawTime = fromTick; drawTime < rightTime + minStep; drawTime += minStep) {
        double onScreenX = double(drawTime) * winContext.viewWindowA
    ...
    if (zeroPos > 0) {
        ...
        winContext.darkBeforeZero = zeroPos;
    } else {
        winContext.darkBeforeZero = -100;
    }
...
//        toTime = rightTime;
//    }
//    long long x = fromTime * winContext.viewWindowA + winContext.viewWindowB;
//    qDebug() << "x" << x;
//    long long y = (toTime) *winContext.viewWindowA + winContext.viewWindowB;
//    qDebug() << "y" << y - x;
...
{
    winContext.mouseLocationY = event->pos().y();
    winContext.mouseLocationX = event->pos().x();
    if (isPress) {
        distance = event->pos().x() - startPressX;
        startPressX = event->pos().x();
        // 暂定可向左拖动1000像素
        if (winContext.viewWindowB + distance < 1000) {
            winContext.viewWindowB = winContext.viewWindowB + distance;
        } else {
            winContext.viewWindowB = 1000;
        }
    }
    update();
}
...
void TimeRuler::PaintMouseMeasureLine(QPainter *p) 
{
    ...
    p->setFont(font);
    QFontMetrics fm = p->fontMetrics();
    if (winContext.ifShowMouseMeasureLine) {
        long long mouseOnTime = (long long) (((double) (winContext.mouseLocationX)
                                              - winContext.viewWindowB)
                                             / winContext.viewWindowA);
        ...
        int showX = 0;
        QString showTime;
        if (winContext.mouseLocationX - line1X > line2X - winContext.mouseLocationX) {
            showX = line2X;
        ...
        if (winContext.pressShift) {
            showX = winContext.mouseLocationX;
        }
        if (showX < right && showX > left) {
            ...
            if (winContext.mouseLocationY > 30) {
                pen.setColor(QColor(0, 0, 0));
                p->setPen(pen);

...
{
    ui->setupUi(this);
    rightTime = new QDateTime;

    QString strDateTime = rightTime->currentDateTime().toString("yyyy-MM-dd hh:mm:ss");
    qDebug() << strDateTime;
...
{
    rightTime->addSecs(1);
    rightTimeInt = rightTime->currentDateTime().toTime_t();
    leftTimeInt = rightTimeInt - tagNumber;
...
void TimeLine::mouseMoveEvent(QMouseEvent *e)
{
    ...
    if(abs(difference) < distanceOfTag){
    ...
    if (showLeftTime + sec >= leftTimeInt) {
        ...
        showRightTime = showRightTime + sec;
    }
